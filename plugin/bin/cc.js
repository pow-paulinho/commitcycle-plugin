...
var init_tools_list = __esm({
  "../../apps/mcp/src/tools-list.ts"() {
    ...
    TOOLS = [
      ...
      {
        name: "cc_status",
        description: "What is on the board: a single task by id, or everything in flight. Read-only. For what is open to you on this machine right now, run `cc status` \u2014 the grant is local and this server deliberately cannot see it.",
        inputSchema: {
          type: "object",
...
function renderAgentsBlock(input) {
  ...
  lines.push(
    ...
    "### Deploys",
    "",
    "Never remove `cc guard-deploy` from a deploy command, and never set",
...
async function runLogin(opts) {
  ...
  try {
    ...
    else log("  no projects yet \u2014 create one in the console, or with `cc project <name>`");
...
`);
    ...
    if (Date.now() > deadline) {
      return { ok: false, failures: [{ field: "code", message: "Nobody confirmed the code in time. Run `cc pair` again." }] };
    }
    await sleep(2e3);
  }
}
...
function resolveBoard(root) {
  ...
  return {
    ...
    /* Last in the chain and first in ordinary use: the session `cc login`
...
function yaml(zones, owner) {
  const lines = [
    ...
    "#",
    "# Written by `cc init`. Edit it freely \u2014 it is yours, and it is meant to grow",
...
async function runInit(opts) {
  ...
  const agents = writeAgentsBlock(root, renderAgentsBlock({
    ...
    // the moment it exists rather than at the next `cc sync` (CC-202).
    board: readBoardBinding(root),
    /* Checked, not assumed (CC-209). `cc init` usually installs the hook and
...
`);
      ...
      log("  Everything local still works \u2014 the hook, the zone map, `cc status`.");
      log("  `cc start`, `cc sync` and `cc submit` will refuse until .zones/board.json names one.");
    }
  } catch {
    log("\n  No board named, so nothing was written for one.");
    log("  Everything installed above still works \u2014 add one later by running `cc init` again.");
  }
  return {
    zonesYml,
    accepted,
...
function renderRecord(t, today) {
  ...
  return [
    ...
    `# ${t.id} \xB7 ${t.title}`,
    "",
    `> **Materialised from the board by \`cc pull --write-missing\` on ${today}.**`,
...
async function runPull(input) {
  ...
  if (res.status === 401 || res.status === 403) {
    log("  the board refused this session \u2014 run `cc login`. Nothing was written.");
...
async function recordDriftCheck(root, board2, fetchImpl = fetch, now = /* @__PURE__ */ new Date()) {
  ...
  try {
    ...
    if (!res.ok) {
      return [{
        ...
        fix: res.status === 401 ? "Run `cc login`. Until the board answers, nothing here can tell a stale record from a correct one." : "Read the board's logs. An unreadable board cannot confirm that the files on disk describe real work."
      }];
    }
  ...
  if (reworded.length) {
    checks.push({
      ...
      fix: "Same work, two names. Harmless until something resolves a task by its title \u2014 push the file's wording with `cc sync`, or edit the board to match."
    });
  }
  if (states.behind.length) {
    checks.push({
      ...
      fix: "Run `cc pull --adopt`. A record that disagrees with the board is the record being stale, not a second source of truth."
    });
  }
  if (states.ahead.length) {
    checks.push({
      ...
      // This check reads state only. `cc pull` reads the events too, which is
      ...
      fix: "Run `cc pull` \u2014 it reads each one's events and says which the board can account for. The ones it cannot are evidence, not staleness: walk them through the gate, or record why they cannot be. Until then the gate reads the board's answer, not the file's."
    });
  }
  ...
  if (notRunning.length) {
    checks.push({
      ...
      fix: 'Nothing is running under these. `cc pause <task> --reason "..."` puts them back in Todo and hands the grant back; access is re-earned at the gate when the work actually starts.'
    });
  }
  ...
  if (orphaned.length) {
    checks.push({
      ...
      fix: "Either they were filed against a different board, or this board is not the one this repository reports to. Check `cc doctor`'s board scope line before assuming the files are wrong."
    });
  }
  ...
  for (const rel of manifests) {
    ...
    try {
      ...
      for (const [name, body] of Object.entries(scripts)) {
        ...
        if (/cc\s+guard-deploy|deploy-guard/.test(body)) continue;
...
function runDoctor(root, now = /* @__PURE__ */ new Date()) {
  ...
  if (!existsSync5(zonesPath)) {
    checks.push({
      ...
      fix: "Run `cc init`. Without it CC has no idea what is protected, so writes are denied."
    });
  } else {
    ...
    if (!parsed.ok) {
      ...
      checks.push({
        ...
        fix: n === 0 ? "Nothing is protected. Run `cc init` to propose some." : void 0
      });
  ...
  if (!existsSync5(settingsPath)) {
    checks.push({
      ...
      fix: "Run `cc init`. Note: without the hook, CC only catches things at merge time."
    });
  } else {
    try {
      ...
      if (!cmd) {
        checks.push({
          ...
          fix: "Run `cc init` to write it, or add the hook by hand."
        });
      } else {
...
The zone map is invalid: ${parsed.issues[0]?.message}`);
    out.push("Writes are denied until it parses. Run `cc doctor`.");
    return out.join("\n");
  }
  out.push("");
  out.push("Zones");
...
function loadDoc(root) {
  ...
  try {
    ...
    return { error: "No .zones/zones.yml here. Run `cc init` first." };
  }
...
function runProtect(root, glob, zoneId) {
  ...
  return {
    ok: true,
    message: `"${glob}" is now in "${zoneId}". The change is in zones.yml, in your diff \u2014 commit it with the work, and run \`cc sync\` so the board learns the new boundary.`
  };
}
function runDismiss(root, glob) {
  ...
  return {
    ok: true,
    message: `"${glob}" is recorded as reviewed-and-open. The closing gate stops asking about it. If that turns out to be wrong, \`cc protect\` it later \u2014 the record of both decisions is the point.`
  };
}
...
function writePlaybooksReadme(root, scan) {
  ...
  const next = [
    ...
    "",
    "<!-- The table is generated by cc sync from the playbook files \u2014 edit them, not this. -->",
    "",
...
function feedOffers(root, taskId, touched) {
  ...
  for (const topic of record.topics) {
    ...
    cc feed ${topic} "what this close taught (${taskId})"   # skipping is legal by rule`
    );
  }
  return offers;
}
...
async function runSeed(input) {
  ...
  for (const draft of body.drafts) {
    ...
    if (existsSync11(join18(root, rel))) {
      ...
      log(`  ${draft.topic}: a playbook already exists \u2014 the seed never overwrites; feed it instead (cc feed)`);
      continue;
    }
...
async function runChallenge(input) {
  ...
  if (!input.ask) {
    ...
    log("");
    log("  Decisions need a human: run `cc challenge` in a terminal to accept or reject these.");
...
function writeDecision(root, n, p, verdict, grounds) {
  ...
  const lines = [
    ...
    `- Date: ${today}`,
    "- Proposed by: challenger (`cc challenge`, D-46)",
...
function runVerify(opts) {
  ...
  if (!taskId) {
    return {
      ...
      failures: [{
        field: "branch",
        message: opts.fromHistory ? "Not on a task branch, and no task named. With --from-history, pass the id: `cc verify --from-history CC-40`." : "Not on a task branch. There is nothing to verify against."
      }]
    };
  }
  ...
  if (!existsSync13(zonesPath)) {
    return {
      ...
      unprotected: [],
      warnings: [],
      failures: [{ field: "zones", message: "No .zones/zones.yml. Run `cc init` first." }]
    };
  }
...
${strays.map((s) => `      ${s.slice(0, 8)} ${subjectOf(root, s).slice(0, 88)}`).join("\n")}
  ...
  if (grant.kind === "none") {
    ...
    extra.push({
      field: "grant",
      message: `The grant at .zones/state/grants/${taskId}.json exists but is not readable JSON, so the declared-zones check did not run here \u2014 it runs at the transition. \`cc sync\` rewrites the grant.`
    });
  } else if (declared.length === 0) {
    log("  the grant on disk opens no protected zones");
    extra.push({
      field: "grant",
      message: "The grant on disk opens zero protected zones, so any change inside one will refuse as undeclared. That is correct if this task declared none \u2014 if the board says otherwise, the grant is stale: run `cc sync` and verify again."
    });
  } else {
...
async function runStart(input) {
  ...
  writeRecord(root, {
    ...
    // task it records is the drift `cc doctor` already counts twelve of.
    title,
...
function writeRecord(root, r) {
  const body = renderTaskRecord({
    ...
    quality_gate: r.gate,
    created: r.today,
    writtenBy: "Record written at birth by `cc start` (CC-111) \u2014 the D-43 orphan pattern\nends where this command begins.",
    history: [
      [r.today, "Triage", "`cc start`"],
...
async function runScope(opts) {
  ...
  if (unknown.length) {
    return {
      ...
      failures: [{
        field: unknown[0],
        message: `cc scope does not write ${unknown.join(", ")}. ` + (unknown.includes("state") ? "A task moves through the gate and nowhere else \u2014 `cc start`, `cc submit` and `cc pause` are the edges (E15)." : `It writes ${SCOPABLE.join(", ")}.`)
      }]
    };
  }
...
var HELP = `cc \u2014 a gate for AI-assisted development

  cc login        Sign in to the board: an address, a six-digit code, done
  cc pair         Join this machine to a board: a code here, confirmed in your
                  browser. Sets up .zones/board.json \u2014 no name to type
  cc connect <ticket>
                  The same, started from the board: paste the line it gave you
  cc guard-deploy Refuse a deploy that is not from the trunk. Put it in front
                  of your deploy command: cc guard-deploy && <your deploy>
  cc init         Set this repository up: propose zones, install the hook
  cc start "title" | <task>   --goal ... --non-goals ... --criteria ... [--zone id:write]
                  [--topics a,b] [--priority high] [--gate none]
                  One command: intake -> scope -> branch -> gate -> grant -> record
  cc mcp          Serve the MCP tools over stdio, for a client's .mcp.json
  cc show [task]  Read a task as the board holds it, or list the board.
                  Read-only \u2014 Triage tasks have no record file to read instead
  cc scope <task> [--title ...] [--goal ...] [--non-goals ...] [--criteria ...]
                  [--zone id:write] [--topics a,b] [--priority high] [--gate none]
                  Correct what a task says. It cannot move it \u2014 that is the gate
  cc status       What task is active, and what is open to it
  cc sync         Push the zone map and events, pull the grant for this branch
  cc pull [--adopt] [--write-missing]
                  Reconcile the record files against the board's task states
  cc submit       Hand the work in: In Progress -> In Review, and close the zones
  cc pause [task] --reason "..."
                  Stop without giving up: In Progress -> Todo, and hand the grant back
  cc request-access [task] --zone <id> --mode read|write --reason "..." --alternative "..."
                  Ask the zone's owner to open it for this task
  cc requests     What is waiting on you \u2014 the zone, the task, why, and what
                  they would do instead
  cc approve <id> [--hours N] \xB7 cc deny <id>
                  Answer one from here. The console is the stronger door; the
                  trail records which one answered
  cc verify [--from-history]
  ...
  cc challenge    The seed's counterweight (D-46): proposals for what this repo
                  lacks, each with a market source. Accept or reject \u2014 both become
                  decision files; there is no --yes, deciding is yours
  cc doctor       Why is CC behaving like this?
...
async function main2() {
  ...
  if (command in NOT_YET) {
    console.error(`\`cc ${command}\` is not built yet \u2014 ${NOT_YET[command]}`);
    return 1;
  }
  const root = repoRoot();
  if (!root) {
  ...
  switch (command) {
    ...
    case "login": {
      ...
      console.log(`
The session is in ${sessionPath()}, and every cc command will use it.
`);
      return 0;
    }
...
asked when someone needs access. Then run \`cc doctor\` to confirm it is live.
`
      );
      if (!existsSync18(join25(root, ".zones", "playbooks"))) {
        console.log(
          `No playbooks yet. Once the board is connected, \`cc seed\` drafts them from
...
`
    ...
    case "start": {
      ...
      if (!first) {
        console.error(
          'cc start needs a title, or the id of a task the board already holds:\n  cc start "what this task does" --goal ... --non-goals ... --criteria ...\n  cc start CC-55 [--goal ...]        starts what is already on the board'
        );
        return 1;
      }
...
${result.taskId} is In Progress on \`${result.branch}\``);
    ...
    case "mcp": {
      ...
      serve(() => {
        const board2 = resolveBoard(root);
        if (!board2.apiUrl) {
          return { error: "No board address. Run `cc init` in this repository, or set CC_API_URL \u2014 the MCP tools all talk to a board." };
        }
        if (!board2.tenant) {
...
`);
        ...
        console.log(`Answer one with \`cc approve ${list[0].id}\` or \`cc deny ${list[0].id}\`.
`);
        return 0;
      }
      const id = args.find((a) => !a.startsWith("--"));
      if (!id) {
        console.error(`cc ${command} needs a request id \u2014 \`cc requests\` lists them.`);
        return 1;
      }
      const req = (pending.body.requests ?? []).find((r2) => r2.id === id);
      if (!req) {
        console.error(`
  ${id} is not waiting on anybody. \`cc requests\` lists what is.
`);
        return 1;
      }
      ...
      if (!me) {
        console.error("\n  Not signed in, so there is nobody to attribute this to. Run `cc login`.\n");
        return 1;
      }
...
${id} ${command === "approve" ? "approved" : "denied"} as ${me}.`);
      ...
      console.log("`cc sync` on the task's branch brings the decision down.\n");
      return 0;
    }
    /* Joining this machine to a board (CC-184, D-53). Before `boardEnv`, like
       `cc login`: pairing is what a repository with no board.json does, and
  ...
  Paired to ${result.tenant}/${result.repo}.`);
      ...
      console.log("  Next: `cc init` if this repository has no zone map yet.\n");
      return 0;
    }
    ...
    case "scope": {
      ...
      if (!taskId) {
        console.error('cc scope needs a task: cc scope CC-12 --goal "..." --non-goals "..."');
        return 1;
      }
      const take = (name) => {
        const i = args.indexOf(`--${name}`);
...
${taskId} updated: ${Object.keys(patch).join(", ")}.`);
    ...
    /* The dashboard-first direction (CC-189). Before `boardEnv` like `cc pair`:
       a repository with no board.json is exactly what this is for. */
    case "connect": {
      const ticket = args.find((a) => !a.startsWith("--"));
      if (!ticket) {
        console.error("cc connect needs the ticket from the board: cc connect <ticket>");
        return 1;
      }
      const at = args.indexOf("--board");
      const apiUrl = (at === -1 ? void 0 : args[at + 1]) ?? resolveBoard(root).apiUrl ?? "https://dash.commitcycle.com";
...
Grant updated for ${result.taskId}. Run \`cc status\` to see what is open.
...
${did}.` + (r.ahead.length ? ` ${r.ahead.length} left alone \u2014 a file ahead of the board is evidence, not staleness.` : "") + " Commit them with the task that ran this.\n"
    ...
    case "pause": {
      ...
      if (!reason.trim()) {
        console.error('cc pause needs --reason "...". A pause that does not say "until what" is indistinguishable from work that quietly stopped.');
        return 1;
      }
...
When it is answered, \`cc sync\` brings the decision down.
`
      );
      return 0;
    }
...
Seeded ${result.written.length} playbook(s): ${result.written.join(", ")}.`);
        console.log("Declare topics at cc start and the gate loads them. Feed them at close \u2014 the loop is theirs now.\n");
...
${result.accepted.length} accepted, ${result.rejected.length} refused knowingly, ${result.skipped} skipped.`);
    ...
    case "feed": {
      ...
      if (!topic || !text) {
        console.error('cc feed needs a topic and a bullet: cc feed backend "what the close taught (CC-xxx)"');
        return 1;
      }
